...
export const classifyImage = async (imageElement: HTMLImageElement): Promise<PredictionResult[]> => {
  try {
    ...
    // Enhanced validation logic
    // Jika ML confidence sangat rendah atau tidak yakin, gunakan Visual Analysis sebagai fallback
    ...
    if (useVisualFallback) {
      ...
      Object.entries(RICE_VISUAL_CHARACTERISTICS).forEach(([riceType, specs]) => {
        let score = 0;
        // Recalculate match based on updated characteristics (simplified here, ideally reuse logic)
        // Brightness check
        ...
        // Color/Surface checks (simplified reuse)
    ...
    } else if (mlConfidence < 0.03 && !generalRiceAnalysis) {
      throw new Error('Objek dalam foto bukan beras. Mohon upload foto beras yang jelas.');
    } else if (mlConfidence < 0.08 && (!riceTypeAnalysis.isValidRiceType || visualConfidence < 0.3)) {
